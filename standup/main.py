...
def _prompt(label: str, default: str = "") -> str:
    ...
    try:
        value = console.input("{0}{1}: ".format(label, suffix)).strip()
...
def _prompt_bool(field_name: str, default: bool) -> bool:
    ...
    while True:
        ...
        console.print("[red]❌ {0}[/red]".format(message))
...
def _post_to_slack(webhook_url: str, text: str) -> None:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            console.print(
                "[red]❌ Slack post failed: {0} {1}[/red]".format(
        ...
        console.print(
            "[red]❌ Slack post error: {0}[/red]".format(sanitize_error_message(exc))
        )
...
def _show_quality_breakdown(quality: Dict[str, object]) -> None:
    ...
    if isinstance(strengths, list) and strengths:
        ...
        for strength in strengths:
            console.print("  [green]• {0}[/green]".format(strength))

    if isinstance(issues, list) and issues:
        console.print("[bold]Issues[/bold]")
        for issue in issues:
            console.print("  [yellow]• {0}[/yellow]".format(issue))
...
[Service]
Type=oneshot
ExecStart=/bin/bash -lc '{0}'

[Install]
WantedBy=default.target
""".format(script_content.strip())
...
def _install_startup(config: dict) -> None:
    ...
    for name, content in definition_content.items():
        ...
        console.print("[cyan]{0}[/cyan] -> {1}".format(name, target))
    ...
    try:
        if sys.platform == "win32":
            subprocess.run(
                ["schtasks", "/Create", "/TN", "StandupBotWarmUp", "/XML", str(paths["definition"]), "/F"],
                check=True,
            )
            ...
            subprocess.run(["systemctl", "--user", "enable", "--now", "standupbot-warmup.service"], check=True)
        console.print("[green]✅ Startup warm-up installed.[/green]")
    except Exception as exc:
        console.print(
            "[yellow]⚠️  Startup files were written, but automatic registration failed: {0}[/yellow]".format(
...
def _run_maintenance() -> None:
    ...
    table.add_row(
        "History cleanup",
        "No cleanup needed" if cleanup_result is None else "{0} row(s) deleted".format(cleanup_result),
    )
...
def _uninstall_startup() -> None:
    ...
    for key in ("script", "definition"):
        if key in paths:
            console.print("[cyan]{0}[/cyan] -> {1}".format(key, paths[key]))
    ...
    try:
        if sys.platform == "win32":
            ...
            subprocess.run(["systemctl", "--user", "disable", "--now", "standupbot-warmup.service"], check=False)
    except Exception:
        pass
...
def run_setup_wizard() -> None:
    ...
    if choice == "2":
        ...
        for index, model_name in enumerate(KNOWN_GROQ_MODELS[:3], 1):
            console.print("  {0}. {1}".format(index, model_name))
        while True:
        ...
        if ok and api_key:
            console.print("[yellow]⚠️  Storing API key in config file. Consider using GROQ_API_KEY env var instead.[/yellow]")
            config["provider"]["groq"]["api_key"] = api_key
        elif not ok:
            console.print("[yellow]⚠️  {0} Leaving API key blank.[/yellow]".format(message))
    else:
        ...
        if provider.is_available():
            ...
            console.print("[yellow]⚠️  Ollama not detected. Install from https://ollama.com, then run: ollama pull {0}[/yellow]".format(config["provider"]["ollama"]["model"]))

    console.print("\n[bold]Repo paths[/bold] (enter one per line, blank to finish):")
    repos = []
    while True:
        raw_path = _prompt("  Repo path {0}".format(len(repos) + 1), "")
        if not raw_path:
            break
        ...
        if ok:
            ...
            console.print("  [green]✅ Added: {0}[/green]".format(repo_path))
        else:
            console.print("  [red]❌ {0}[/red]".format(message))
    config["repos"] = repos
    ...
    if ok:
        ...
        console.print("[yellow]⚠️  {0} Leaving Slack webhook blank.[/yellow]".format(message))
...
def main() -> None:  # noqa: C901
    ...
    history_parser.add_argument("--limit", type=lambda value: validate_positive_int_arg("--limit", value, 1, 100), default=10, help="Number of history entries to show")
    history_parser.add_argument("--clear", action="store_true", help="Clear history entries")
    history_parser.add_argument("--days", type=lambda value: validate_positive_int_arg("--days", value, 1, 3650), help="Only clear entries older than this many days")

    warmup_parser = subparsers.add_parser("warm-up", help="Pre-load the configured model")
    warmup_parser.add_argument("--install-startup", action="store_true", help="Install warm-up to run automatically at login")
    warmup_parser.add_argument("--uninstall-startup", action="store_true", help="Remove startup warm-up integration")
    ...
    parser.add_argument("--provider", type=validate_provider_arg, metavar="NAME", help="Provider override: ollama or groq (one-time, does not change config)")
    ...
    parser.add_argument("--maintenance", action="store_true", help="Run safe local maintenance tasks")

    args = parser.parse_args()

    if args.version:
        console.print("StandupBot v{0}".format(__version__))
        return
        ...
        for entry in entries:
            ...
            for key in sorted(entry.keys()):
                if key in ("ts", "event", "level"):
                    continue
                detail_parts.append("{0}={1}".format(key, entry[key]))
            table.add_row(
    ...
    if args.command == "models":
        ...
        if models:
            ...
            for model_name in models:
                console.print("  • {0}".format(model_name))
        else:
    ...
    if args.command == "history":
        ...
        if args.clear:
            description = "entries older than {0} days".format(args.days) if args.days else "all history entries"
            if not _confirm_action("Delete {0}?".format(description)):
                console.print("[yellow]History clear cancelled.[/yellow]")
                return
            deleted = clear_history(args.days)
            noun = "entry" if deleted == 1 else "entries"
            console.print("[green]✅ Deleted {0} history {1}.[/green]".format(deleted, noun))
            return
        ...
        for entry in entries:
            ...
            table.add_row(str(entry.get("created_at", ""))[:16], repos, str(entry.get("provider", "")), preview)
        console.print(table)
        return
    ...
    console.print("[dim]Using {0}...[/dim]".format(provider.get_provider_name()))
    ...
    if not all_commits:
        console.print("[yellow]No commits found in the last {0} hours. Did you take a day off?[/yellow]".format(hours))
        return
    ...
    processed_commits = filter_and_classify_commits(all_commits) if filtering_enabled else annotate_commits(all_commits)
    if filtering_enabled and not processed_commits:
        console.print("[yellow]⚠️  All commits matched the default noise filter. Falling back to unfiltered commits for this run.[/yellow]")
    ...
    cached_entry = None if args.no_cache else find_cached_standup_entry(fingerprint, tone, provider_slug)
    ...
    if cached_entry:
        ...
        quality["score"] = int(cached_entry.get("quality_score", 0) or 0)
        used_cache = True
        cache_time = str(cached_entry.get("created_at", ""))[11:16]
        console.print("[dim]⚡ Using cached standup from {0}[/dim]".format(cache_time))
    else:
        ...
        try:
            if config.get("quality", {}).get("enabled", True):
                generation = generate_with_quality_retry(prompt, provider, tone, int(config.get("quality", {}).get("min_score", 0)), max_retries=2)
            ...
            console.print(
                "[red]❌ {0}[/red]".format(sanitize_error_message(exc))
            )
            sys.exit(1)
    ...
    final_output = _render_final_output(raw_standup_text, template_name, config, processed_commits, provider_slug)
    ...
    if config.get("quality", {}).get("enabled", True) or int(quality.get("score", 0) or 0) > 0:
        score = int(quality.get("score", 0) or 0)
        console.print("Quality Score: {0} {1}/100".format(format_score_badge(score), score))
    ...
    if args.copy:
        try:
            ...
            console.print(
                "[yellow]⚠️  Clipboard copy failed: {0}[/yellow]".format(
                    sanitize_error_message(exc)
                )
            )
