...
- `security.py`: `redact_sensitive_patterns`'s private-IP pattern for the
  ...
  to Groq. The `172.16-31.x.x` and `192.168.x.x` branches were unaffected. Fixed
  to match all 4 octets for every range.
- `security.py`: the password/secret/token redaction pattern only matched a
  single whitespace-delimited token after `=`/`:`, so a quoted multi-word secret
  (e.g. `password: "my secret phrase"`) was only partially redacted. Now also
  matches quoted values in full.
- `config.py`: `_deep_merge` returned a shallow copy, so when `override` did not
  touch a given key, the result shared the *same* nested dict objects as the
  module-level `_DEFAULTS`. `load_config()`'s in-place assignment of the Groq API
  key from the `GROQ_API_KEY` env var could therefore mutate `_DEFAULTS` itself,
  leaking the key into later `load_config()` calls in the same process even after
  the env var was unset. `_deep_merge` now deep-copies, so the result never
  shares references with `base`.
- `main.py`: the Linux systemd unit generated by `--install-startup` embedded a
  multi-line script directly into a single `ExecStart=` line. Confirmed via
  `systemd-analyze verify` that this produced an invalid unit
  ("Unbalanced quoting" / "Unit configuration has fatal error, unit will not be
  started"). Fixed by writing the warm-up script to its own file (matching the
  existing Windows `.ps1` approach) and pointing `ExecStart=` at that file.
- `main.py`: the Windows scheduled-task XML declared `encoding="UTF-16"` but was
  written to disk via `write_text(..., encoding="utf-8")`, an encoding mismatch
  that risks Task Scheduler rejecting or misparsing the file. The XML declaration
  now says `UTF-8`, matching what is actually written.
- `main.py`: `enforce_rate_limit()` ran before the standup-cache lookup, so a
  cache hit (no real LLM call) could still be hard-blocked by a cooldown left
  over from a previous real generation. Moved the rate-limit check into the
  cache-miss branch only.
- `classifier.py` / `formatter.py`: every `COMMIT_TYPES` emoji was an empty
  string, which combined with `formatter.py`'s fixed `" {emoji} ("` spacing to
  produce a double space in every prompt line (e.g. `"- [09:32]  (fix) ..."`).
  Added emoji for `feat`/`fix` and made the emoji prefix conditional so no
  double space appears for types without one.
- `setup.py`, `README.md`, `CHANGELOG.md`: actually removed the UTF-8 BOM this
  time. The v0.2.4 entries below claiming the BOM was removed from `setup.py`
  and `README.md` were inaccurate — the BOM was present in both files (and in
